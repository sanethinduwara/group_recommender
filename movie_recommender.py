...
class MovieRecommender(object):
    ...
    def get_recommendation_by_id(self, id, type="user"):
        if type == "user":
            return self._get_user_recommendations(id, k=10)
        if type == "group":
            return self._get_group_recommendations(id, k=10)


    def _get_user_recommendations(self, user_id, k=None):
        ...
        _unrated_movie_ids = _unrated_movies[1][0:len(_unrated_movies[0])-1]
        _unrated_user_ids = _unrated_movies[0][0:len(_unrated_movies[0])-1]

        _predicted_ratings = self.model(_unrated_user_ids, _unrated_movie_ids)
        _predicted_ratings = _predicted_ratings * 5

        _top_recommendations = self._get_top_k_recommendations(_unrated_movie_ids, _predicted_ratings.cpu().detach().numpy(), k)
    ...
    def _get_top_k_recommendations(self, movies, ratings, k=None):
        ...
        # _reshaped_prediction = _reshaped_prediction.astype(np.int64)
        # _reshaped_prediction.sort(key=lambda x:x[1])
